...
const CheckoutScreen = () => {
  ...
  const handleApplyVoucher = async () => {
    ...
    setVoucher(null);
    try {
      const res = await fetchVoucherByCode({ userId, voucherCode: voucherCode.trim() });
      if (res?.data) {
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (responseCreatePaymentIntent.error) {
        Alert.alert("Lỗi", responseCreatePaymentIntent.error);
        return;
      }
      ...
      if (paymentError) {
        Alert.alert("Lỗi thanh toán", paymentError.message);
        return;
      }
      await Promise.all(items.map((item) => paymentCart(item.id)));
    ...
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scroll}>
        <View style={styles.section}>
          ...
          <View style={styles.inputReadonly}>
            <Text style={styles.inputReadonlyText}>{name}</Text>
          </View>
          ...
          <AddressPicker
            onSelect={(area) => {
              setAreaAddress(area);
              setAddress((detailAddress ? detailAddress + ", " : "") + area);
            }}
          />
          ...
          {voucher ? (
            <View style={styles.voucherSuccess}>
              ...
              <Text style={styles.voucherSuccessText}>
                Áp dụng thành công! Giảm {formatCurrency(voucher.discount_amount || 0)}
              </Text>
            </View>
          ) : null}
        </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.paymentMethod}>
            <Icon name="card" size={20} color="#2563eb" />
            <Text style={styles.paymentMethodText}>Thẻ tín dụng / Ghi nợ (Stripe)</Text>
            <Icon name="checkmark-circle" size={18} color="#16a34a" style={{ marginLeft: "auto" }} />
          </View>
        </View>
      ...
      <View style={styles.footer}>
        ...
        >
          {loading
            ? <ActivityIndicator size="small" color="#fff" />
            : <Text style={styles.payButtonText}>Đặt hàng & Thanh toán</Text>
          }
        </TouchableOpacity>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  paymentMethodText: { fontSize: 14 * scaleWidth, color: "#1e293b", fontWeight: "600" },
  footer: {
    backgroundColor: "#fff",
    padding: 16 * scaleWidth,
